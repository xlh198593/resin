...
router.all('/', function(req, res, next) {
	try {
		...
		logger.info("财务接口 requestParams %:::%",requestParams)
		...
		if(typeof requestParams.app_token != 'undefined') {
			...
			bizUtils.validAppToken(req, res, requestParams.app_token, function(tokenValidData) {
				...
				if('finance.voucherRewardByMobile' == requestParams.service) {	// 根据手机号赠送礼券
					...
					request.post(appConfig.financeshellPath, {form: cardFormData}, function(error, response, cardBody) {
						...
						try {
							if(!error && response.statusCode == 200) {
								...
								request.post(appConfig.financeshellPath, {form: payFormData}, function(error, response, payBody) {
					...
					logger.info('财务接口返回数据 %::%',JSON.stringify(body));
					request.post(appConfig.financeshellPath, {form: formData}, function(error, response, body) {
						try {
							if(!error && response.statusCode == 200) {	// 请求成功后直接返回查询结果
								logger.info('财务接口结束 %::%',body);
								res.send(JSON.parse(body));
							} else {
								errHandler.systemError(res, error);
							}
			...
			bizUtils.validUserToken(req, res, requestParams.user_token, function(tokenValidData) {
				...
				// if(requestParams.paymethods="h5") {
				// 	var formData = {
				// 		service : "order.consumer.h5OrderCreate",
				// 		params : requestParams.params
				// 	};
				// 	console.log("h5createorder:::",formData)
					...
					request.post(appConfig.notificationPath, {form: validSMSCodeFormData}, function(error, response, validBody) {
						try {
							if(!error && response.statusCode == 200) {
								...
								request.post(appConfig.financeshellPath, {form: voucherExchangeFormData}, function(error, response, exchangeBody) {
									try {
										if(!error && response.statusCode == 200) {
