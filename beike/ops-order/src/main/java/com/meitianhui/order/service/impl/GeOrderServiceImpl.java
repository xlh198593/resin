...
@Service
public class GeOrderServiceImpl implements GeOrderService {
	...
	@Autowired
	private GeOrderDao geOrderDao;

	@Autowired
	private OrderService orderService;

	@Override
	public void handleGeOrderPayCreateNotify(Map<String, Object> paramsMap, ResultData result)
		...
		GeOrderItem orderItem = new GeOrderItem();
		...
		geOrderDao.insertGeOrderItem(orderItem);
		// 扣库存
		...
		geOrderDao.insertGeOrder(order);
	...
	@Override
	public void geOrderDetailFind(Map<String, Object> paramsMap, ResultData result)
			throws BusinessException, SystemException, Exception {
		...
		for (GeOrder order : orderList) {
			...
			List<GeOrderItem> orderItemList = geOrderDao.selectGeOrderItem(tempMap);
			for (GeOrderItem orderItem : orderItemList) {
	...
	@Override
	public void geOrderListFindForOp(Map<String, Object> paramsMap, ResultData result)
			throws BusinessException, SystemException, Exception {
		...
		List<Map<String, Object>> orderList = geOrderDao.selectGeOrderListForOp(paramsMap);
	...
	@Override
	public void handleGeOrderDelivered(Map<String, Object> paramsMap, ResultData result)
			throws BusinessException, SystemException, Exception {
		ValidateUtil.validateParams(paramsMap, new String[] { "order_id", "data_source" });
		List<GeOrder> GeOrderList = geOrderDao.selectGeOrder(paramsMap);
		if (GeOrderList.size() == 0) {
			throw new BusinessException(RspCode.ORDER_NOT_EXIST, RspCode.MSG.get(RspCode.ORDER_NOT_EXIST));
		}
		GeOrder order = GeOrderList.get(0);
		...
		int updateFlag = geOrderDao.updateGeOrder(paramsMap);
		if (updateFlag == 0) {
	...
	@Override
	public void handleGeOrderReceived(Map<String, Object> paramsMap, ResultData result)
			throws BusinessException, SystemException, Exception {
		ValidateUtil.validateParams(paramsMap, new String[] { "order_id", "data_source" });
		List<GeOrder> GeOrderList = geOrderDao.selectGeOrder(paramsMap);
		if (GeOrderList.size() == 0) {
	...
	@Override
	public void geOrderEdit(Map<String, Object> paramsMap, ResultData result)
			throws BusinessException, SystemException, Exception {
		try {
			...
			geOrderDao.updateGeOrder(paramsMap);
	...
	@Override
	public void geOrderSettlementCount(Map<String, Object> paramsMap, ResultData result)
			throws BusinessException, SystemException, Exception {
		...
		// 统计金额和数量
		Map<String, Object> settlementCountMap = geOrderDao.selectGeOrderSettlementCount(paramsMap);
	 ...
	 */
	@Override
	public void handleGeOrderSettlement(Map<String, Object> paramsMap, ResultData result)
			throws BusinessException, SystemException, Exception {
		...
		paramsMap.put("status", Constant.ORDER_RECEIVED);
		List<Map<String, Object>> settlementList = geOrderDao.selectGeOrderDetailForSettlement(paramsMap);
		...
		for (String supplier_id : settlementMap.keySet()) {
			...
			for (Map<String, Object> tempMap : tempList) {
				...
				int updateFlag = geOrderDao.updateGeOrder(bizParams);
				if (updateFlag == 0) {
